...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAEOqQNoRExxZ98DPVCvkV8oQ1SOxkAjQQAC4xgAAhMJKFYlTkzeysOKTDYE")
        await m.delete()
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('BACKUP', url="https://t.me/MovieTimesTV")
                    ],[
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton("BACKUP", url="t.me/MovieTimesTV")
                ],[
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MovieTimesTV  ' + formate_file_name(file.file_name)
    ...
    files = files_[0]
    title = '@MovieTimesTV ' + formate_file_name(files.file_name)
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@MovieTimesTV  {formate_file_name(files.file_name)}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
